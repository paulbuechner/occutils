...
export function getPrData(pr: PullRequest): PrData | undefined {
  ...
  const match = content.match(/## occutils@(?<version>\d.+)/);
  ...
  const body = [
    ...
    `description: Explore the changelog for occutils version ${version}. Learn about the latest features, bug fixes, and improvements.`,
...
export async function getLatestPr(): Promise<PullRequest> {
  const { data } = await octokit.pulls.list({
    ...
    repo: "occutils",
    base: "main",
    head: "paulbuechner:changeset-release/main",
    per_page: 1,
  });
...
export async function getMergedPrs(): Promise<PullRequests> {
  const { data } = await octokit.pulls.list({
    ...
    repo: "occutils",
    base: "main",
    head: "occutils:changeset-release/main",
    per_page: 100,
  });
