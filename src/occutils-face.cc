...
TopoDS_Face FromWire(const TopoDS_Wire& wire)
{
  ...
  const BRepBuilderAPI_MakeFace faceMaker(wire);
...
std::optional<gp_Ax1> Normal(const TopoDS_Face& face, double u, double v, double precision)
{
  const auto surface = surface::FromFace(face);
