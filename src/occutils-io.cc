 ...
 */
static std::string _IFSelectReturnStatusToString(const IFSelect_ReturnStatus code)
{
  switch (code)
  {
...
{
  if (auto readStat = reader->ReadFile(filename.c_str()); readStat != IFSelect_RetDone)
  {
...
TopoDS_Shape ReadOneShape(const std::shared_ptr<XSControl_Reader>& reader)
{
  ...
  if (const auto numRoots = reader->NbRootsForTransfer(); numRoots < 1)
  {
