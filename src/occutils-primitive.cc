...
TopoDS_Solid MakeBox(const double            xSize,
                     const double            ySize,
                     const double            zSize,
                     const PositionCentering center,
                     gp_Pnt                  origin)
{
...
TopoDS_Solid MakeCube(const double size, const PositionCentering center, const gp_Pnt& origin)
{
  return MakeBox(size, size, size, center, origin);
}

TopoDS_Solid MakeCone(const gp_Ax1& axis,
                      const double  diameter1,
                      const double  diameter2,
                      const double  length,
                      const bool    centerLength)
{
  gp_Pnt origin = axis.Location();
  if (centerLength)
  {
    origin.Translate(gp_Vec(axis.Direction()).Multiplied(-length / 2.0));
  }

  BRepPrimAPI_MakeCone builder(gp_Ax2(origin, axis.Direction()), diameter1, diameter2, length);
  return builder.Solid();
}

TopoDS_Solid MakeCylinder(const double      diameter,
                          const double      length,
                          const Orientation orientation,
                          PositionCentering center,
                          gp_Pnt            origin)
{
  // Compute offsets based on centering
