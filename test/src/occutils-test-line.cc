...
TEST(test_line, LineTest_LineParallel2d)
{
  ...
  const gp_Lin2d linX(gp_Pnt2d(0, 0), occutils::direction::X2d());
  EXPECT_TRUE(occutils::line::IsParallel(linX, linX));

  // A line should be parallel to its reverse line
  const gp_Lin2d linMinusX = linX.Reversed();
  ...
  const gp_Lin2d linY(gp_Pnt2d(0, 0), occutils::direction::Y2d());
...
TEST(test_line, LineTest_LineIntersection2D)
{
  ...
  const gp_Lin2d lin1(gp_Pnt2d(0, 0), occutils::direction::X2d());
  auto           result = occutils::line::Intersection(lin1, lin1);
  EXPECT_FALSE(result.has_value());

  // Two intersecting lines should have an intersection point
  const gp_Lin2d lin2(gp_Pnt2d(0, -1), occutils::direction::Y2d());
  ...
  const gp_Lin2d lin3(gp_Pnt2d(0, 1), occutils::direction::X2d());
